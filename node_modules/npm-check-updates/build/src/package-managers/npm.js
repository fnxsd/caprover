...
const greatest = async (packageName, currentVersion, options = {}) => {
    ...
    (0, filter_1.default)(versions, filterPredicate(options))
        .map(o => o.version)
...
const distTag = async (packageName, currentVersion, options = {}) => {
    const revision = (await viewOne(packageName, `dist-tags.${options.distTag}`, currentVersion, options)); // known type based on dist-tags.latest
...
const newest = async (packageName, currentVersion, options = {}) => {
    ...
    const versionsSatisfyingNodeEngine = (0, filter_1.default)(result.versions, version => (0, filters_1.satisfiesNodeEngine)(version, options.nodeEngineVersion)).map((o) => o.version);
    ...
    const versionsWithTime = (0, pullAll_1.default)(versions, TIME_FIELDS);
    return ((0, last_1.default)(options.pre !== false ? versions : versionsWithTime.filter(version => !versionUtil.isPre(version))) || null);
};
...
const minor = async (packageName, currentVersion, options = {}) => {
    const versions = (await viewOne(packageName, 'versions', currentVersion, options));
    return versionUtil.findGreatestByLevel((0, filter_1.default)(versions, filterPredicate(options)).map(o => o.version), currentVersion, 'minor');
};
exports.minor = minor;
/**
...
const patch = async (packageName, currentVersion, options = {}) => {
    const versions = (await viewOne(packageName, 'versions', currentVersion, options));
    return versionUtil.findGreatestByLevel((0, filter_1.default)(versions, filterPredicate(options)).map(o => o.version), currentVersion, 'patch');
};
exports.patch = patch;
